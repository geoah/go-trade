...
func trade(cmd *cobra.Command, args []string) {
	...
	if ast, cur, err := market.GetBalance(); err != nil {
		logrus.WithError(err).Fatalf("Could not get first time balance")
	} else {
		logrus.
	...
	// go func() {
	// 	for sig := range c {
	// 		logrus.WithField("sig", sig).Infof("Interrupted")
	// 		now := time.Now()
	// 		data := []*mrk.Candle{}
	...
	// 		logrus.Warnf("Completed trade with %d actions", trader.Trades)
	// 		os.Exit(0)
	// 	}
	// }()
